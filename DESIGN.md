...
2. Manager (메인 세션, commands/whip.md + prompts/manager.md)
   ...
   → Executor를 Agent 서브에이전트로 디스패치 (bypassPermissions)
...
task_dir: "/tmp/whipper-2026-03-24-경쟁사-분석"
status: running
---
...
Main Session (Manager)
  └─ Executor (Agent, bypassPermissions, general-purpose)
...
[
  {
    ...
    "path": "/tmp/whipper-2026-03-24-경쟁사-분석",
    "started_at": "2026-03-24T09:00:00Z",
...
작업별 `/tmp/whipper-*`에 실제 파일, `~/.claude/whipper-logs/index.json`에 전체 인덱스.
...
- Slack은 `config/slack.json` 또는 아래 환경변수로 설정한다.
  - `WHIPPER_SLACK_WORKSPACE_DOMAIN`
  - `WHIPPER_SLACK_BOT_TOKEN`
  - `WHIPPER_SLACK_APP_TOKEN`
  - `WHIPPER_SLACK_BOT_USER_ID`
- Notion은 `config/notion.json` 또는 아래 환경변수로 설정한다.
  - `WHIPPER_NOTION_TOKEN`
  - `WHIPPER_NOTION_DATABASE_ID`
  - `WHIPPER_NOTION_VIDEO_DATABASE_ID`
- launchd 배포는 tracked plist를 직접 수정하지 않고 `python3 scripts/daemon/install_launch_agent.py --reload` 로 현재 머신용 plist를 렌더링한다.
...
2. **현재 프로젝트 이력**: `~/.claude/whipper-logs/index.json` 또는 최근 `/tmp/whipper-*` 작업 폴더를 읽어 명령, 결과, iterations 요약
