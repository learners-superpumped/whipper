...
### 4.3단계: 평가 결과 Notion 기록

notion_page_id가 있으면, **상세한** 평가 결과를 Notion 페이지에 기록한다:

```bash
python3 ${CLAUDE_PLUGIN_ROOT}/scripts/notion/append_log.py "$NOTION_PAGE_ID" "## Iteration {N} 평가 — {ALL PASS 또는 FAIL}
| 기준 | 판정 | 근거 |
|------|------|------|
| {기준1} | PASS/FAIL | {구체적 근거: 어디서 확인했는지, 출처 URL, 불통과 사유} |
...
**원칙**: Notion 페이지만 보면 각 iteration의 평가 맥락을 완전히 이해할 수 있어야 한다.
...
- 하나라도 FAIL:
  ...
  2. notion_page_id가 있으면:
     ```bash
     python3 ${CLAUDE_PLUGIN_ROOT}/scripts/notion/update_status.py "$NOTION_PAGE_ID" --iteration {N}
     ```
